...
export default function Cpu() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Section title="Как устроен тюнинг" sub="Каким путём приложение управляет железом и почему выбран именно он">
        <div className="grid grid-cols-3 gap-3 text-[12.5px] text-ink-2 leading-relaxed">
          <div className="panel-2 p-3">
            ...
            <ul className="list-disc ml-4 flex flex-col gap-1">
              <li>EXPO, PBO и Curve Optimizer живут в BIOS. Из Windows их не переключить, не поставив кернел-драйвер Ryzen Master, — от него отказались осознанно.</li>
              <li>Поэтому приложение <b className="text-ink">меряет до и после</b>: свои тесты плюс фактическая частота под нагрузкой. Значения для BIOS подсказывает Claude, вводите их вы.</li>
              <li>Температура берётся из ACPI-интерфейса платы — того же, через который работают вентиляторы.</li>
            </ul>
          </div>
          <div className="panel-2 p-3">
            <div className="text-ink font-semibold mb-1">Видеокарта RTX 5070 Ti</div>
            <ul className="list-disc ml-4 flex flex-col gap-1">
              <li>Лимит мощности, смещения частот и режим вентиляторов — напрямую через NVAPI, без сторонних утилит.</li>
              <li>Драйвер держит эти значения только до перезагрузки, поэтому любой сбой откатывает их сам. На этом и построен автоподбор.</li>
              <li>Ступенчатая проверка со сверкой контрольных сумм: ловятся не только вылеты, но и тихие ошибки вычислений.</li>
            </ul>
          </div>
          <div className="panel-2 p-3">
            <div className="text-ink font-semibold mb-1">Вентиляторы и память</div>
            <ul className="list-disc ml-4 flex flex-col gap-1">
              <li>Вентиляторы платы — через штатный ACPI-интерфейс. Драйвер WinRing0, на котором работает FanControl, входит в блоклист Microsoft и здесь не используется.</li>
              <li>Скважность ШИМ считает кривая Smart Fan из BIOS; приложение управляет тем, что она получает на вход.</li>
              <li>Память проверяется записью и чтением цепочки — так видно нестабильность после включения EXPO, которая портит данные, но не роняет систему.</li>
            </ul>
          </div>
        </div>
      </Section>
